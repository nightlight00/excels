...
using excels.Items.Placeable.Decorations.Torches;

namespace excels.Tiles.Decorations.Torches
{
	#region Granite
	internal class GranTorchT : ModTile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			ItemDrop = ModContent.ItemType<GranTorch>();
			DustType = ModContent.DustType<Dusts.EnergyDust>();
			...
			AddMapEntry(new Color(79, 75, 134), name);

			// Assets
			if (!Main.dedServ)
			{
				flameTexture = ModContent.Request<Texture2D>("excels/Tiles/Decorations/Torches/GranTorchT_Flame");
			}
		}
		...
		public override float GetTorchLuck(Player player)
		{
			...
			bool inExampleUndergroundBiome = Main.LocalPlayer.ZoneGranite;
			return inExampleUndergroundBiome ? 0.5f : 0f; // ExampleTorch gives maximum positive luck when in example biome, otherwise a small negative luck
		}
	...
	#region Stellar
	internal class StellarBulb : ModTile
	{
		private Asset<Texture2D> flameTexture;
		...
		public override void SetStaticDefaults()
		{
			...
			ItemDrop = ModContent.ItemType<StellarTorch>();
			DustType = -1;
			AdjTiles = new int[] { TileID.Torches };
			...
			AddMapEntry(new Color(255, 158, 46), name);

			// Assets
			if (!Main.dedServ)
			{
				flameTexture = ModContent.Request<Texture2D>("excels/Tiles/Decorations/Torches/StellarBulb_Flame");
			}
		}
		...
		public override float GetTorchLuck(Player player)
		{
			...
			bool inExampleUndergroundBiome = Main.LocalPlayer.ZoneSkyHeight;
			return inExampleUndergroundBiome ? 0.8f : 0f; // ExampleTorch gives maximum positive luck when in example biome, otherwise a small negative luck
		}
